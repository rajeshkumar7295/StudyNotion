...
const NestedView = ({ handleChangeEditSectionName }) => {
  ...
  const handleDeleteSection = async (sectionId) => {
    console.log('handleDeleteSection token.......',token);
  ...
  return (
            ...
            <div className="px-6 pb-4">
              {
                section?.subSection?.map((data) => (
                  <div key={data._id} className="flex cursor-pointer items-center justify-between gap-x-3 border-b-2 border-b-richblack-600 py-2" onClick={() => setViewSubSection(data)}>
                    ...
                    <div onClick={(e) => e.stopPropagation()} className='flex item-center gap-x-3 '>
    ...
    {
      confirmationalModal ? (<ConfirmationModal
        modalData={confirmationalModal}
      />):(<></>)
    }
    </>
  )
